...
project's code. No external APIs or scripts — 4 subagents (agent `general`),
each blind to the others, each with a fixed lens.
...
**Honest framing (never drop this from the final output):** the 4 members run
on the same model playing different roles. They share prior knowledge, so
...
Raw input received: $ARGUMENTS

## Step -1 — Detect mode

If `--impl` is present anywhere in the input above, this is **Implementation
...
From the input above, take the text following `--q` (quoted or not) as the
user's question. If there is no `--q` but there is text, use all of it as the
question.

**If the input is completely empty** (you ran `/use-council` with nothing
...
These two questions go straight into the chat as plain text, not via the
`question` tool — just two simple conversational turns before Step 1.

## Step 1 — Up to 5 context questions

Before analyzing, use the `question` tool to surface context the code alone
...
interacts in Spanish. Each question needs concrete options (the user can also
type a free-text answer). Don't ask about roles or output format — those are
fixed by this command's design.
...
1. If the user pointed at specific files/folders, read them.
2. Otherwise, search the project (glob/grep) for relevant files from keywords
   in the question (function names, paths, domain terms). Cap at ~5-8 files
   so you don't overload each role's context.
...
Launch all 4 with the `task` tool (agent `general`), in parallel — a single
batch of invocations, not sequential — so they run concurrently and blind to
each other.
...
Wait for all 4 subagents to finish. If one fails, note it and continue with
the rest — don't relaunch the call.

## Step 5 — Display each perspective

First show this banner, verbatim, in Spanish:

> **Consejo local** — las 4 perspectivas vienen del mismo modelo interpretando
> roles distintos, no de proveedores de IA diferentes. Trata las coincidencias
> entre roles como un punto de partida común a cuestionar, no como validación
> independiente.
...
`X` en un servicio dedicado" is). If, after filtering, there are zero
actionable items, tell the user in Spanish and stop — don't invent work.
...
2. Use the `question` tool with a single question: "¿Implementar esta
...
3. If accepted: implement it now, following this project's existing
   conventions (thin, minimal, no unrelated refactors). Read/edit files as
   needed. After implementing, show a short summary of what changed (files
